...
const applyNumberRange = (variant: any, range: [number, number]): any => {
  const [minNum, maxNum] = range;
  const sequence = variant.sequence;
  
  // Pokud sekvence je v rozsahu, vrať ji beze změny
  const minInSequence = Math.min(...sequence);
  const maxInSequence = Math.max(...sequence);
  
  if (minInSequence >= minNum && maxInSequence <= maxNum) {
    return variant;
  }
  
  // Posun sekvence do požadovaného rozsahu
  const offset = minNum - minInSequence;
  const newSequence = sequence.map(num => num + offset);
  
  // Kontrola, že je stále v rozsahu
  const newMax = Math.max(...newSequence);
  if (newMax > maxNum) {
    // Zkrátí sekvenci pokud přesahuje maximum
    const validSequence = newSequence.filter(num => num <= maxNum);
    if (validSequence.length >= 5) {
      return {
        ...variant,
...
const DraggableNumber: React.FC<DraggableNumberProps> = ({ number, id, onDrop, style, size }) => {
...
interface DropSlotProps {
  index: number;
  number: number | null;
  expectedNumber?: number; // Očekávan�� číslo pro validaci
...
const DropSlot: React.FC<DropSlotProps> = ({ index, number, expectedNumber, onDrop, onRemove, size, isMissing }) => {
  ...
  const backgroundColor = isEmpty ? 
    ...
    isMissing ? 
      (isCorrect ? COLORS.FILLED_BG : isIncorrect ? '#FFE4E1' : COLORS.SLOT_BG) : 
      COLORS.SLOT_BG;
  
  const border = isEmpty ? 
    `3px dashed ${COLORS.SLOT_LINE}` : 
    isMissing ? 
      (isCorrect ? `3px solid ${COLORS.FILLED_BORDER}` : isIncorrect ? `3px solid ${COLORS.RED}` : `3px solid ${COLORS.SLOT_LINE}`) :
  ...
  return (
    <div
      ...
      style={{
        ...
        fontWeight: 'bold',
        color: isMissing ? COLORS.GREEN : COLORS.CRIMSON,
...
export function NumberSequenceGame({ settings = {}, onSwitchGame }: NumberSequenceGameProps) {
  ...
  const generateNewVariant = useCallback(() => {
    // Filtrace variant podle nastavení
    let availableVariants = SEQUENCE_VARIANTS;
    
    if (settings.sequenceTypes && settings.sequenceTypes.length > 0) {
      availableVariants = SEQUENCE_VARIANTS.filter(variant => {
        const sequence = variant.sequence;
        const type = determineSequenceType(sequence);
        return settings.sequenceTypes.includes(type);
      });
    }
    
    // Pokud žádné varianty neodpovídají, použij všechny
    if (availableVariants.length === 0) {
      availableVariants = SEQUENCE_VARIANTS;
    }
    
    let variant = availableVariants[Math.floor(Math.random() * availableVariants.length)];
    
    // Aplikace délky sekvence z nastavení
    if (settings.sequenceLength) {
      ...
      if (variant.sequence.length !== targetLength) {
        variant = generateCustomSequence(variant, targetLength);
      }
    }
    
    // Aplikace rozsahu čísel z nastavení
    if (settings.numberRange) {
      variant = applyNumberRange(variant, settings.numberRange);
    }
    
    let missing = generateMissingNumbers(variant.sequence);
    
    // Aplikace počtu chybějících čísel z nastavení
    if (settings.missingCount) {
    ...
    if (allFilled) {
      if (allCorrect) {
        ...
        setTimeout(() => {
          ...
          // Automaticky generovat novou variantu po krátké pauze
          setTimeout(() => {
            setIsComplete(false);
            generateNewVariant();
          }, 500);
  ...
  }, [userAnswers, currentSpec, generateNewVariant, playSound]);
      ...
      <div className="w-full h-screen flex flex-col" style={{ backgroundColor: COLORS.PAGE_BG }}>
        ...
        <div className="w-full bg-[#F5E6D0] py-6 px-4">
          <div className="flex flex-col items-center space-y-4 max-w-4xl mx-auto">
            ...
            {/* Popis pod názvem */}
            <div className="text-xl md:text-2xl font-medium text-[#4e5871] text-center">
              {currentSpec?.title || "Načítání..."}
            </div>
