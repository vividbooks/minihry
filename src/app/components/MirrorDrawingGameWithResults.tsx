...
interface MirrorDrawingGameSettings {
  ...
  useResultScreen?: boolean; // Nová možnost pro zapnutí/vypnutí nových obrazovek
}
...
export function MirrorDrawingGameWithResults({ settings = {} }: MirrorDrawingGameProps) {
  ...
  const gameConfig = {
    ...
    useResultScreen: settings.useResultScreen ?? true // Defaultně zapnuto
  };
  ...
  // Nové stavy pro výsledkové obrazovky
  const [showResultScreen, setShowResultScreen] = useState(false);
  const [lastResultWasSuccess, setLastResultWasSuccess] = useState(false);
  ...
  useEffect(() => {
    if (isEvaluated && currentLevel) {
      ...
      if (gameConfig.useResultScreen) {
        // Zobraz výsledkovou obrazovku
        setLastResultWasSuccess(isComplete);
        setShowResultScreen(true);
      } else if (isComplete && gameConfig.autoProgression) {
        // Původní chování - automaticky načti další level
  ...
  const handleResultScreenContinue = () => {
    setShowResultScreen(false);
    
    if (lastResultWasSuccess && gameConfig.autoProgression) {
      // Pokud byl úspěch, načti další level
      generateNewLevel();
    }
    // Pokud nebyl úspěch, nechej hráče zkusit znovu (jen schová obrazovku)
  };
    ...
    >
      ...
      <div className="hidden lg:flex h-screen">
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          ...
          {/* Výsledek - pouze pokud nepoužíváme nové obrazovky */}
          {isEvaluated && !gameConfig.useResultScreen && (
      ...
      {/* Nová výsledková obrazovka */}
      {showResultScreen && gameConfig.useResultScreen && (
        <GameResultScreen
          isSuccess={lastResultWasSuccess}
          onContinue={handleResultScreenContinue}
          autoHideDuration={lastResultWasSuccess ? 3 : undefined} // Auto-hide pouze pro úspěch
