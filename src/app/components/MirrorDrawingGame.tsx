import React, { useState, useEffect } from 'react';
import { GameGrid } from './GameGrid';
import { CompactControls } from './CompactControls';
import { Cell, evaluateGrid } from '../utils/mirrorDrawingLogic';
import { RandomShape, generateRandomLevel, Difficulty, APP_COLORS, COLORS } from '../constants/mirrorDrawingData';
import { GameResultScreen } from './GameResultScreen';

// Typy nastavení hry
interface MirrorDrawingGameSettings {
  difficulty?: 'easy' | 'medium' | 'hard';
  gridSizes?: number[];
  objectCount?: [number, number];
  enableEraser?: boolean;
  autoProgression?: boolean;
  progressionDelay?: number;
  showFeedback?: boolean;
  enableDragToDraw?: boolean;
  backgroundColor?: string;
}

interface MirrorDrawingGameProps {
  settings?: MirrorDrawingGameSettings;
}

export function MirrorDrawingGame({ settings = {} }: MirrorDrawingGameProps) {
  // Aplikování nastavení s fallback na výchozí hodnoty
  const gameConfig = {
    difficulty: (settings.difficulty as Difficulty) ?? Difficulty.EASY,
    autoProgression: settings.autoProgression ?? true,
    progressionDelay: (settings.progressionDelay ?? 2) * 1000, // převod na ms
    showFeedback: settings.showFeedback ?? true,
    enableDragToDraw: settings.enableDragToDraw ?? true,
    backgroundColor: settings.backgroundColor ?? APP_COLORS.PAGE_BACKGROUND
  };
  const [currentLevel, setCurrentLevel] = useState<RandomShape | null>(null);
  const [selectedColor, setSelectedColor] = useState(COLORS[1]);
  const [rightGrid, setRightGrid] = useState<Record<string, Cell>>({});
  const [isEvaluated, setIsEvaluated] = useState(false);
  const [difficulty, setDifficulty] = useState<Difficulty>(gameConfig.difficulty);
  const [showSuccessScreen, setShowSuccessScreen] = useState(false);

  // Generuj první náhodný tvar při načtení
  useEffect(() => {
    generateNewLevel();
  }, [difficulty]);

  // Auto-progression po úspěšném dokončení - zobrazí success screen s videem
  useEffect(() => {
    if (isEvaluated && currentLevel) {
      const cells = Object.values(rightGrid);
      const correctCells = cells.filter(cell => cell.isCorrect === true).length;
      const totalCells = cells.length;
      const isComplete = correctCells === totalCells && totalCells > 0;
      
      if (isComplete) {
        // Zobrazit success screen s videem
        const timer = setTimeout(() => {
          setShowSuccessScreen(true);
        }, 500); // Krátké zpoždění pro plynulý přechod
        
        return () => clearTimeout(timer);
      }
    }
  }, [isEvaluated, rightGrid, currentLevel]);

  const generateNewLevel = () => {
    const newLevel = generateRandomLevel(difficulty);
    setCurrentLevel(newLevel);
    handleReset();
  };

  const handleCellClick = (row: number, col: number) => {
    if (isEvaluated || !currentLevel) return;
    
    // Pouze pravá polovina je klikatelná
    const halfGrid = Math.floor(currentLevel.gridSize / 2);
    if (col < halfGrid) return;
    
    const key = `${row},${col}`;
    const isEraser = selectedColor === COLORS[COLORS.length - 1]; // Poslední barva je guma
    
    setRightGrid(prev => {
      if (isEraser) {
        // Guma - odstraň buňku
        const newGrid = { ...prev };
        delete newGrid[key];
        return newGrid;
      } else {
        // Kreslení - přidej barvu
        return {
          ...prev,
          [key]: { color: selectedColor }
        };
      }
    });
  };

  const handleEvaluate = () => {
    if (!currentLevel) return;
    
    const newRightGrid = evaluateGrid(rightGrid, currentLevel);
    setRightGrid(newRightGrid);
    setIsEvaluated(true);
  };

  const handleReset = () => {
    setRightGrid({});
    setIsEvaluated(false);
    setShowSuccessScreen(false);
  };

  const handleContinueAfterSuccess = () => {
    setShowSuccessScreen(false);
    generateNewLevel();
  };

  // Zobraz loading dokud se negeneruje první level
  if (!currentLevel) {
    return (
      <div 
        className="flex items-center justify-center min-h-screen"
        style={{ backgroundColor: gameConfig.backgroundColor }}
      >
        <div className="text-center">
          <div className="text-4xl mb-4">🎲</div>
          <p style={{ color: APP_COLORS.TASK_TEXT }}>Generuji náhodný obrázek...</p>
        </div>
      </div>
    );
  }

  // Zkontroluj jestli je level dokončený
  const cells = Object.values(rightGrid);
  const correctCells = cells.filter(cell => cell.isCorrect === true).length;
  const totalCells = cells.length;
  const isComplete = correctCells === totalCells && totalCells > 0 && isEvaluated;

  // Popisky pro zrcadlové kreslení
  const labels = {
    left: 'VZOR',
    right: '✏️ DOKRESLI ZRCADLOVĚ',
    mode: 'ZRCADLENÍ',
    modeIcon: '🪞',
    description: 'Dokresli pravou polovinu jako zrcadlový obraz'
  };

  return (
    <div 
      className="min-h-screen w-full"
      style={{ backgroundColor: gameConfig.backgroundColor }}
    >
      {/* Desktop layout - horizontální */}
      <div className="hidden lg:flex h-screen">
        {/* Mřížka vlevo */}
        <div className="flex-1 flex flex-col min-w-0 h-full">
          
          {/* Popisky pro zrcadlové kreslení */}
          <div className="flex justify-center pb-1 w-full mt-24">
            <div className="flex justify-between w-full max-w-2xl">
              <div className="text-center flex-1">
                <p className="text-4xl" style={{ color: APP_COLORS.TASK_TEXT }}>
                  {labels.left}
                </p>
              </div>
              <div className="text-center flex-1">
                <p className="text-4xl" style={{ color: APP_COLORS.TASK_TEXT }}>
                  {labels.right}
                </p>
              </div>
            </div>
          </div>
          
          <div className="flex-1 w-full overflow-hidden">
            <GameGrid 
              level={currentLevel}
              rightGrid={rightGrid}
              onCellClick={handleCellClick}
              isEvaluated={isEvaluated}
              selectedColor={selectedColor}
            />
          </div>
          

        </div>

        {/* Ovládací prvky vpravo - vertikálně vycentrované */}
        <div className="flex items-center justify-center p-6">
          <div className="flex flex-col items-center gap-6">
            {/* Výrazné vyhodnocení nahoře */}
            {isEvaluated && (
              <div className="bg-white rounded-2xl p-6 shadow-xl border-4 min-w-[200px] text-center"
                   style={{ 
                     borderColor: isComplete ? '#4CAF50' : '#F7A800',
                     backgroundColor: isComplete ? '#E8F5E8' : '#FFF8E1'
                   }}>
                {isComplete ? (
                  <div>
                    <div className="text-6xl mb-3">🎉</div>
                    <p className="text-2xl font-bold mb-2" style={{ color: '#4CAF50' }}>
                      Perfektní!
                    </p>
                    <p className="text-lg" style={{ color: '#4CAF50' }}>
                      Zrcadlový obraz je správný!
                    </p>
                    {gameConfig.autoProgression && (
                      <p className="text-sm text-gray-600 mt-3">
                        Další level se načte automaticky...
                      </p>
                    )}
                  </div>
                ) : (
                  <div>
                    <div className="text-5xl mb-3">🤔</div>
                    <p className="text-xl font-bold mb-2" style={{ color: '#F7A800' }}>
                      Zkus to znovu!
                    </p>
                    <p className="text-base text-gray-600">
                      Porovnej si svůj obrázek se vzorem 🪞
                    </p>
                  </div>
                )}
              </div>
            )}
            
            <CompactControls
              selectedColor={selectedColor}
              onColorSelect={setSelectedColor}
              difficulty={difficulty}
              onDifficultyChange={setDifficulty}
              onEvaluate={handleEvaluate}
              onReset={handleReset}
              onNewLevel={generateNewLevel}
              isEvaluated={isEvaluated}
              gridSize={currentLevel.gridSize}
            />
          </div>
        </div>
      </div>

      {/* Mobile layout - vertikální */}
      <div className="lg:hidden flex flex-col min-h-screen">
        
        {/* Mřížka nahoře */}
        <div className="flex-1 flex flex-col p-2">
          {/* Popisky pro zrcadlové kreslení */}
          <div className="flex justify-center py-1 w-full">
            <div className="flex justify-between w-full max-w-sm">
              <div className="text-center flex-1">
                <p className="text-sm" style={{ color: APP_COLORS.TASK_TEXT }}>
                  {labels.left}
                </p>
              </div>
              <div className="text-center flex-1">
                <p className="text-sm" style={{ color: APP_COLORS.TASK_TEXT }}>
                  {labels.right}
                </p>
              </div>
            </div>
          </div>
          
          <div className="flex-1 w-full overflow-hidden">
            <GameGrid 
              level={currentLevel}
              rightGrid={rightGrid}
              onCellClick={handleCellClick}
              isEvaluated={isEvaluated}
              selectedColor={selectedColor}
            />
          </div>
          
          {/* Výrazné vyhodnocení */}
          {isEvaluated && (
            <div className="py-3 px-4">
              <div className="bg-white rounded-xl p-4 shadow-lg border-2 text-center mx-auto max-w-xs"
                   style={{ 
                     borderColor: isComplete ? '#4CAF50' : '#F7A800',
                     backgroundColor: isComplete ? '#E8F5E8' : '#FFF8E1'
                   }}>
                {isComplete ? (
                  <div>
                    <div className="text-4xl mb-2">🎉</div>
                    <p className="text-lg font-bold mb-1" style={{ color: '#4CAF50' }}>
                      Perfektní!
                    </p>
                    <p className="text-sm" style={{ color: '#4CAF50' }}>
                      Zrcadlový obraz je správný!
                    </p>
                    {gameConfig.autoProgression && (
                      <p className="text-xs text-gray-600 mt-2">
                        Další level se načte automaticky...
                      </p>
                    )}
                  </div>
                ) : (
                  <div>
                    <div className="text-3xl mb-2">🤔</div>
                    <p className="text-base font-bold mb-1" style={{ color: '#F7A800' }}>
                      Zkus to znovu!
                    </p>
                    <p className="text-xs text-gray-600">
                      Porovnej si svůj obrázek se vzorem 🪞
                    </p>
                  </div>
                )}
              </div>
            </div>
          )}
        </div>

        {/* Ovládací prvky dole */}
        <div className="p-4 flex justify-center">
          <div className="flex flex-col items-center gap-4">
            <CompactControls
              selectedColor={selectedColor}
              onColorSelect={setSelectedColor}
              difficulty={difficulty}
              onDifficultyChange={setDifficulty}
              onEvaluate={handleEvaluate}
              onReset={handleReset}
              onNewLevel={generateNewLevel}
              isEvaluated={isEvaluated}
              gridSize={currentLevel.gridSize}
            />
          </div>
        </div>
      </div>

      {/* GameResultScreen s videem - zobrazí se po úspěšném dokončení */}
      {showSuccessScreen && (
        <GameResultScreen
          isSuccess={true}
          onContinue={handleContinueAfterSuccess}
          successText="SKVĚLÉ! SPRÁVNÝ ZRCADLOVÝ OBRAZ!"
          showContinueButton={true}
          displayType="gameComplete"
        />
      )}
    </div>
  );
}